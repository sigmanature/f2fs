...
retry:
		...
		if (mapping_writably_mapped(mapping))
			flush_dcache_folio(folio);
		// /*my_debug*/
		// printk(KERN_EMERG "iomap_write_iter: folio->index %d,folio nr pages %d",folio->index , folio->_nr_pages);
		/*my_debug*/
		 ...
		 * no stale data is exposed.  Onlcdy once that's done can we
 ...
 */
bool iomap_writeback_handle_eof(struct folio *folio, struct inode *inode,
		u64 *end_pos)
{
...
EXPORT_SYMBOL_GPL(iomap_writeback_handle_eof);
