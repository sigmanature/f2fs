...
static void f2fs_is_folio_writeback(struct folio *folio)
{
    if (folio_test_writeback(folio))
        printk(KERN_EMERG "folio %p,index %d,order %d host inode ino %d is already in writeback state", folio);
}
static void f2fs_check_inode_folios_writeback(struct inode *inode)
{
    ...
    xas_for_each(&xas, folio, end) {
        if (!xa_is_value(folio)) {
            if (folio_test_writeback(folio)) {
                printk(KERN_EMERG "Error:inode(%lu): folio %p, index %lu, order %u is in writeback state",
            ...
            {
                printk(KERN_EMERG "Error:inode(%lu): folio %p, index %lu, order %u is locked",
                    inode->i_ino, folio, folio_index(folio),
                    folio_order(folio));
            }
            else
            {
                printk(KERN_EMERG "inode(%lu): folio %p, index %lu, order %u is not locked",
                    inode->i_ino, folio, folio_index(folio),
                    folio_order(folio));
            }
        }
...
static void print_wbp(struct folio*folio)
{
    ...
    {
    printk(KERN_EMERG "folio index %lu, order %d, host inode ino %d,write_bytes_pending %u",folio_index(folio), folio_order(folio),inode->i_ino,atomic_read(&fifs->write_bytes_pending));
    }
}
static void print_rbp(struct folio*folio)
{
    ...
    if(fifs&&folio_order(folio)>0)
    {
        printk(KERN_EMERG "folio index %lu, order %d, host inode ino %d,rbp %d rbp minus magic %u",folio_index(folio), folio_order(folio),inode->i_ino,fifs->read_bytes_pending,fifs->read_bytes_pending-F2FS_IFS_MAGIC);
    }
}
static void f2fs_list_folios_bio(struct bio *bio)
{
    ...
    bio_for_each_folio_all(fi, bio) {
        ...
        fifs = folio->private;
        if (fifs&&folio_order(folio)>0) {
            pr_debug("in bio %p,folio %p, index %lu, order %d, host inode ino %d, fi.length:%d,write_bytes_pending %u",
        ...
        {
            pr_debug("in bio %p,folio %p, index %lu, order %d, host inode ino %d, fi.length:%d",
                bio,folio, folio_index(folio), folio_order(folio),
                folio->mapping->host->i_ino,fi.length);
        }
    }
...
{
    struct inode *inode = folio->mapping->host;
    printk(KERN_EMERG "folio %p, index %lu, order %d, host inode ino %d",
             folio, folio_index(folio), folio_order(folio), inode->i_ino);
}
static void noinline print_folio_mapping(struct folio *folio)
{
    printk(KERN_EMERG "folio %p, index %lu, order %d, host mapping %p",
...
static void inline f2fs_list_folios_cc(struct compress_ctx *cc)
{
    ...
    printk(KERN_EMERG "Begin Listing folios in compress_ctx %p", cc);
    if (!cc || !cc->rpages) {
        printk(KERN_EMERG "compress_ctx or rpages is NULL");
        return;
    }
    ...
    printk(KERN_EMERG "End Listing folios in compress_ctx %p", cc);
}
static void inline f2fs_list_folios_cic(struct compress_io_ctx *cc)
{
    int i;
    unsigned int num_to_skip = 0;
    printk(KERN_EMERG "Begin Listing folios in compress_io_ctx %p", cc);
    if (!cc || !cc->rpages) {
        printk(KERN_EMERG "compress_io_ctx or rpages is NULL");
        return;
    }
    /* Iterate through the cluster pages */
    num_to_skip = 1; // Reset num_to_skip for the first iteration
    ...
    printk(KERN_EMERG "End Listing folios in compress_io_ctx %p", cc);
}
...
{
    printk(KERN_EMERG "in f2fs_print_dn_blkaddr\n");
    dump_stack();
    printk(KERN_EMERG "node_base addr is %p:",get_dnode_addr(dn->inode, dn->node_folio));
    printk(KERN_EMERG "data_blkaddr is %d:",le32_to_cpu(*(get_dnode_addr(dn->inode, dn->node_folio) + dn->ofs_in_node)));
    printk(KERN_EMERG "index:%d ofs_in_node:%d blk_addr: %lx",index,dn->ofs_in_node,dn->data_blkaddr);
}
#endif /*F2FS_DBG_H*/
