...
static inline unsigned long f2fs_folio_private_flags(const struct folio *folio)
{
...
static inline void f2fs_folio_or_private_flags(struct folio *folio,
						unsigned long flags)
{
	if (folio_has_ffs(folio)) {
		struct f2fs_folio_state *ffs = folio->private;
...
{									\
	unsigned long priv = f2fs_folio_private_flags(folio);		\
...
{									\
	unsigned long v = (1UL << PAGE_PRIVATE_NOT_POINTER) |		\
			     (1UL << PAGE_PRIVATE_##flagname);		\
	f2fs_folio_or_private_flags(folio, v);				\
}									\
...
{
	unsigned long data = f2fs_folio_private_flags(folio);
...
static inline void folio_set_f2fs_data(struct folio *folio, unsigned long data)
{
	...
	f2fs_folio_or_private_flags(folio, data);
}
